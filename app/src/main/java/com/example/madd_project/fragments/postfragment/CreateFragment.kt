            ...
            storageRef.child(id).putFile(it).addOnSuccessListener { task->
                task.metadata!!.reference!!.downloadUrl
                    .addOnSuccessListener { url->
                        Toast.makeText(context,"image has been saved successfully", Toast.LENGTH_SHORT).show()
                        val imgUrl = url.toString()
                        firebaseReference.child(id).setValue(Posts(id,name,dueDate,description,quantity,imgUrl))
                            .addOnCompleteListener{
                                Toast.makeText(context,"Post has been saved successfully", Toast.LENGTH_SHORT).show()
                            }
